...
d3d12_create_framebuffer(api_context *ctx, api_image *colorbuf, api_image *zbuf,
...
static api_shader *
d3d12_create_shader(api_context *ctx, const char *source, api_shader_type type)
{
...
d3d12_create_gfx_pipeline(api_context *ctx, const api_gfx_pipeline_desc *desc)
{
...
static void
d3d12_bind_gfx_pipeline(api_context *ctx, api_gfx_pipeline *pipeline)
{
}
...
static void
d3d12_begin_render_pass(api_context *ctx, const api_render_pass_desc *desc)
{
}

static void
d3d12_end_render_pass(api_context *ctx)
{
}

static void
d3d12_bind_vertex_buffers(api_context *ctx, api_buffer *vb, const uint64_t *vb_offsets)
{
}

static void
d3d12_draw(api_context *ctx, const api_draw_desc *desc)
{
}
...
static void
d3d12_upload_buffer_data(api_context *ctx, api_buffer *buf, uint64_t offset, uint64_t size,
                      const void *data)
{
}

static void
d3d12_image_write_png(api_context *ctx, api_image *image, unsigned layer, const char *filename)
{
}

api_context *
d3d12_create_context(const program_options *options)
{
   ...
   ctx->create_shader = d3d12_create_shader;
   ctx->destroy_shader = NULL;

   ctx->create_gfx_pipeline = d3d12_create_gfx_pipeline;
   ctx->destroy_gfx_pipeline = NULL;
   ctx->bind_gfx_pipeline = d3d12_bind_gfx_pipeline;

   ctx->begin_cmdbuf = d3d12_begin_cmdbuf;
   ctx->end_cmdbuf_and_submit = d3d12_end_cmdbuf_and_submit;

   ctx->begin_render_pass = d3d12_begin_render_pass;
   ctx->end_render_pass = d3d12_end_render_pass;
